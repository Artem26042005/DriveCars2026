...
function getUSAClearance(carPrice, auctionFee, yearRelease, engineCapacity, fuelType) {
...
function calculateUSA() {
    ...
    const engineCapacity = Number(document.getElementById('engine-capacity-USA').value);
    const yearRelease = Number(document.getElementById('year-USA').value);

    if (!carPrice) return alert('Введи вартість авто');
    if (!engineCapacity && americaFuel !== 2) return alert('Введи об’єм двигуна');
    if (!yearRelease) return alert('Введи рік випуску');
    ...
    const service = 500;
    const button = 120;
    const clearance = getUSAClearance(carPrice, auctionFee, yearRelease, engineCapacity, americaFuel);
    ...
    document.getElementById('usa-total').textContent = Number(allPrice).toFixed(2);
    document.getElementById('usa-pay').textContent = Number(paymentThree).toFixed(2) + ' $';
}

function calculateEurope(type = 1) {
    ...
    const priceCar = Number(document.getElementById('price-Europe').value);
    const priceEurope = Number(document.getElementById('delivery-Europe').value);
    const yearRelease = Number(document.getElementById('year-Europe').value);
    const engineCapacity = Number(document.getElementById('engine-capacity-Europe').value);

    if (!priceCar) return alert('Введи вартість авто');
    if (!priceEurope) return alert('Введи доставку по Європі');
    if (type === 1 && !yearRelease) return alert('Введи рік випуску');
    if (type === 1 && !engineCapacity && europeFuel !== 2) return alert('Введи об’єм двигуна');

    const priceService = 550;
    const priceCarAuction = priceCar + crossBorder[europeCountry] + processingDocs[europeCountry];
    const broker = 250;
    const certificate = 100;
    const priceColl = collection[europeCountry];
    ...
    document.getElementById('europe-total').textContent = Number(allPriceCar).toFixed(2);
    document.getElementById('europe-pay').textContent = Number(paymentThree).toFixed(2) + ' €';
}
...
function initSimpleSelects() {
    document.querySelectorAll('.simple-select').forEach(select => {
        ...
        trigger.addEventListener('click', () => {
            document.querySelectorAll('.select.open').forEach(s => { if (s !== select) s.classList.remove('open'); });
        ...
        options.forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                if (target === 'usa-fuel') americaFuel = value;
                if (target === 'europe-country') europeCountry = value;
                if (target === 'europe-fuel') europeFuel = value;
            });
        });
    });
}

function initUSAStateSelect() {
    const wrap = document.getElementById('usa-state-select');
    const trigger = wrap.querySelector('.select-trigger');
    const label = document.getElementById('usa-state-label');
    const options = document.getElementById('usa-state-options');
    const search = document.getElementById('usa-state-search');

    function render(list) {
        options.innerHTML = '';
        list.forEach(item => {
            const index = statesInfo.findIndex(x => x.state === item.state);
            const btn = document.createElement('button');
            btn.type = 'button';
            btn.className = 'option' + (index === americaState ? ' active' : '');
            btn.textContent = item.state;
            btn.addEventListener('click', () => {
                americaState = index;
                label.textContent = item.state;
                wrap.classList.remove('open');
                render(filteredStates(search.value.trim()));
            });
            options.appendChild(btn);
        });
    }

    function filteredStates(q) {
        if (!q) return statesInfo;
        return statesInfo.filter(x => x.state.toLowerCase().includes(q.toLowerCase()));
    }

    trigger.addEventListener('click', () => {
        document.querySelectorAll('.select.open').forEach(s => { if (s !== wrap) s.classList.remove('open'); });
        wrap.classList.toggle('open');
        search.focus();
    });

    search.addEventListener('input', () => render(filteredStates(search.value.trim())));
    render(statesInfo);
}
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    initSimpleSelects();
    initUSAStateSelect();
    document.getElementById('btn-calc-USA').addEventListener('click', calculateUSA);
