const statesInfo = [
    ...
    {state: "ABILENE - TX", price: 520, port: 3},
    {state: "ACE - Carson (CA)", price: 445, port: 4},
    {state: "ACE - Perris - CA", price: 410, port: 4},
    {state: "ACE - Perris 2 - CA", price: 410, port: 4},
    {state: "ADELANTO-CA", price: 420, port: 4},
    {state: "AKRON-CANTON (OH)", price: 700, port: 1},
    {state: "ALBANY - NY", price: 425, port: 1},
    {state: "ALBUQUERQUE - NM", price: 870, port: 3},
    {state: "ALDEN NY", price: 645, port: 1},
    {state: "ALTOONA - PA", price: 590, port: 1},
    {state: "AMARILLO - TX", price: 595, port: 3},
...
const SETTINGS = Object.freeze({
  ...
  europeBroker: 250,
  europeService: 200,
  europeCertificate: 100
});

let usaMode='with', europeMode='with';
const money=(v,c)=>`${Number(v).toFixed(2)} ${c}`;
const containerPrice=port=>({1:650,2:700,3:800,4:1200,5:850,6:750,7:1350,8:1200})[port]||0;
const age=year=>Math.min(15,Math.max(1,new Date().getFullYear()-Number(year)));
function exciseBase(fuel,capacity){ if(fuel==='petrol') return capacity<=3000?50:100; if(fuel==='diesel') return capacity<=3500?75:150; return 0; }
function customsBreakdown(carPrice, auctionFee, deliveryForCustoms, year, capacity, fuel){
  if(fuel==='electric'){
    const total=Math.max(0,Number(capacity));
    return {customsValue:carPrice+auctionFee+deliveryForCustoms,duty:0,excise:total,vat:0,total};
  }
  const customsValue=carPrice+auctionFee+deliveryForCustoms;
  const excise=exciseBase(fuel,capacity)*(capacity/1000)*age(year);
  const duty=customsValue*0.10;
  const vat=(customsValue+duty+excise)*0.20;
  return {customsValue,duty,excise,vat,total:duty+excise+vat};
}
function profitFor(base,min){ return Math.max(min,base*SETTINGS.profitPercent/100); }
function swiftGross(base){ const total=base/(1-SETTINGS.swiftPercent/100); return {swift:total-base,total}; }
function line(label,value,c){return `<div class="line"><span>${label}</span><strong>${money(value,c)}</strong></div>`}
function customsHtml(c,currency){ return `<div class="customs"><div class="custom-item"><small>Митна вартість</small><strong>${money(c.customsValue,currency)}</strong></div><div class="custom-item"><small>Мито 10%</small><strong>${money(c.duty,currency)}</strong></div><div class="custom-item"><small>Акциз</small><strong>${money(c.excise,currency)}</strong></div><div class="custom-item"><small>ПДВ 20%</small><strong>${money(c.vat,currency)}</strong></div></div><div class="pay-note">Разом розмитнення: <strong>${money(c.total,currency)}</strong></div>`; }
function renderResult(rootId, currency, total, first, later, rows, customs, mode, firstText){
 const root=document.getElementById(rootId); root.classList.remove('hidden');
 root.innerHTML=`<div class="summary-grid"><div class="card summary highlight"><small>Загальна вартість</small><strong>${money(total,currency)}</strong></div><div class="card summary"><small>Оплата протягом 3 днів</small><strong>${money(first,currency)}</strong></div><div class="card summary"><small>Оплата пізніше</small><strong>${money(later,currency)}</strong></div></div><div class="card details"><h2>Деталізація</h2>${rows.join('')}${mode==='with'?`<h2 style="margin-top:22px">Розмитнення — по пунктах</h2>${customsHtml(customs,currency)}`:''}<div class="pay-note">${firstText}</div></div>`;
}

function calculateUSA(){
 const car=Number(document.getElementById('usa-price').value), idx=Number(document.getElementById('usa-state').value), fuel=document.getElementById('usa-fuel').value, year=Number(document.getElementById('usa-year').value), capacity=Number(document.getElementById('usa-engine').value);
 if(!(car>0&&year>0&&capacity>0)) return alert('Заповніть вартість авто, рік та двигун/батарею');
 const state=statesInfo[idx], auction=getAuctionAmerica(car), land=state.price, sea=containerPrice(state.port);
 const customs=customsBreakdown(car,auction,1600,year,capacity,fuel), customsTotal=usaMode==='with'?customs.total:0;
 const commercialBase=car+auction+land+sea+SETTINGS.usaUnload+SETTINGS.usaUkraine+SETTINGS.usaBroker+SETTINGS.usaService;
 const profit=profitFor(commercialBase,SETTINGS.usaMinProfit);
 // У США весь чистий прибуток має бути оплачений у першому платежі.
 const visibleLotAuction=car+auction+profit;
 const firstBase=visibleLotAuction;
 const swift=swiftGross(firstBase);
 const firstPayment=swift.total;
 const later=land+sea+SETTINGS.usaUnload+SETTINGS.usaUkraine+SETTINGS.usaBroker+SETTINGS.usaService+customsTotal;
 const total=firstPayment+later;
 const rows=[line('Вартість лоту + аукціонний збір',visibleLotAuction,'$'),line('Доставка до порту',land,'$'),line('Доставка по морю',sea,'$'),line('Вигрузка',SETTINGS.usaUnload,'$'),line('Доставка до України',SETTINGS.usaUkraine,'$')];
 if(usaMode==='with') rows.push(line('Розмитнення',customs.total,'$'));
 rows.push(line('Брокерські послуги',SETTINGS.usaBroker,'$'),line('Вартість послуг з підбору та бронювання',SETTINGS.usaService,'$'),line(`SWIFT переказ (${SETTINGS.swiftPercent}%)`,swift.swift,'$'));
 renderResult('usa-results','$',total,firstPayment,later,rows,customs,usaMode,'Протягом 3 днів: лот + аукціонні збори + весь чистий прибуток + SWIFT. Решта витрат сплачується пізніше.');
}

function calculateEurope(){
 const car=Number(document.getElementById('eu-price').value), country=EUROPE_COUNTRIES[Number(document.getElementById('eu-country').value)], delivery=Number(document.getElementById('eu-delivery').value), fuel=document.getElementById('eu-fuel').value, year=Number(document.getElementById('eu-year').value), capacity=Number(document.getElementById('eu-engine').value);
 if(!(car>0&&delivery>=0&&year>0&&capacity>0)) return alert('Заповніть усі поля');
 const customs=customsBreakdown(car,country.fee,300+delivery,year,capacity,fuel), customsTotal=europeMode==='with'?customs.total:0;
 const commercialBase=car+country.fee+delivery+SETTINGS.europeUkraine+SETTINGS.europeBroker+SETTINGS.europeService+SETTINGS.europeCertificate;
 const profit=profitFor(commercialBase,SETTINGS.europeMinProfit);
 // Доставку по Європі і 70% прибутку ховаємо в лот+збір; 30% прибутку — у доставку до України.
 const visibleLotAuction=car+country.fee+delivery+profit*0.70;
 const visibleUkraine=SETTINGS.europeUkraine+profit*0.30;
 const firstCommercial=visibleLotAuction+visibleUkraine+SETTINGS.europeBroker+SETTINGS.europeService+SETTINGS.europeCertificate;
 const swift=swiftGross(firstCommercial);
 const firstPayment=swift.total;
 const later=customsTotal;
 const total=firstPayment+later;
 const rows=[line('Вартість лоту + аукціонний збір',visibleLotAuction,'€'),line('Доставка до України',visibleUkraine,'€')];
 if(europeMode==='with') rows.push(line('Розмитнення',customs.total,'€'));
 rows.push(line('Брокерські послуги',SETTINGS.europeBroker,'€'),line('Вартість послуг з підбору та бронювання',SETTINGS.europeService,'€'),line('Сертифікація',SETTINGS.europeCertificate,'€'),line(`SWIFT переказ (${SETTINGS.swiftPercent}%)`,swift.swift,'€'));
 renderResult('eu-results','€',total,firstPayment,later,rows,customs,europeMode,'Протягом 3 днів сплачується все, окрім розмитнення. Прибуток повністю включений у цей перший платіж.');
}

function init(){
 document.querySelectorAll('.tab').forEach(b=>b.onclick=()=>{document.querySelectorAll('.tab').forEach(x=>x.classList.remove('active'));document.querySelectorAll('.panel').forEach(x=>x.classList.remove('active'));b.classList.add('active');document.getElementById('panel-'+b.dataset.tab).classList.add('active')});
 document.querySelectorAll('.mode').forEach(g=>g.querySelectorAll('.mode-btn').forEach(b=>b.onclick=()=>{g.querySelectorAll('.mode-btn').forEach(x=>x.classList.remove('active'));b.classList.add('active');if(g.dataset.scope==='usa')usaMode=b.dataset.mode;else europeMode=b.dataset.mode;}));
 const us=document.getElementById('usa-state'); statesInfo.forEach((s,i)=>us.add(new Option(s.state,i))); const las=statesInfo.findIndex(s=>s.state.includes('LAS VEGAS')); if(las>=0)us.value=las;
 const eu=document.getElementById('eu-country'); EUROPE_COUNTRIES.forEach((c,i)=>eu.add(new Option(`${c.name} (${c.code})`,i))); eu.value=2;
 const toggleFuel=(sel,label,input)=>{const electric=sel.value==='electric';label.childNodes[0].nodeValue=electric?'Ємність батареї, кВт·год':'Об’єм двигуна, см³';input.placeholder=electric?'Напр. 75':'2000';};
 const uf=document.getElementById('usa-fuel'), ul=document.getElementById('usa-engine-label'), ui=document.getElementById('usa-engine'); uf.onchange=()=>toggleFuel(uf,ul,ui);
 const ef=document.getElementById('eu-fuel'), el=document.getElementById('eu-engine-label'), ei=document.getElementById('eu-engine'); ef.onchange=()=>toggleFuel(ef,el,ei);
 document.getElementById('calc-usa').onclick=calculateUSA; document.getElementById('calc-europe').onclick=calculateEurope;
}
