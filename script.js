...
function paymentCard(title, rows, total, currency, accent=false){
  return `<div class="card payment-card ${accent?'payment-primary':''}"><div class="payment-head"><h2>${title}</h2><strong>${money(total,currency)}</strong></div><div class="payment-lines">${rows.join('')}</div></div>`;
}
function renderResult(rootId, currency, total, first, second, firstRows, secondRows){
 const root=document.getElementById(rootId); root.classList.remove('hidden');
 root.innerHTML=`<div class="payments-grid">${paymentCard('1-й платіж',firstRows,first,currency,true)}${paymentCard('2-й платіж',secondRows,second,currency,false)}</div><div class="card grand-total"><span>Загальна вартість</span><strong>${money(total,currency)}</strong></div>`;
}
...
function calculateUSA(){
 ...
 // США: в першому платежі забираємо весь чистий прибуток, але не показуємо його окремо.
 // Лот + аукціонний збір мають лише +100$ до фактичної суми.
 // Решта прибутку маскується як часткова оплата доставки до порту та морської доставки.
 const lotAuctionMarkup=100;
 const remainingProfit=Math.max(0,profit-lotAuctionMarkup);
 const partialLand=remainingProfit*0.40;
 const partialSea=remainingProfit-partialLand;
 const visibleLotAuction=car+auction+lotAuctionMarkup;
 const firstCommercial=visibleLotAuction+partialLand+partialSea;
 const swift=swiftGross(firstCommercial);
 const firstPayment=swift.total;

 // У другому платежі стоять повні фактичні логістичні витрати.
 const secondPayment=land+sea+SETTINGS.usaUnload+SETTINGS.usaUkraine+SETTINGS.usaBroker+SETTINGS.usaService+customsTotal;
 const total=firstPayment+secondPayment;

 const firstRows=[
   line('Вартість лоту + аукціонний збір',visibleLotAuction,'$'),
   line('Часткова оплата доставки до порту',partialLand,'$'),
   line('Часткова оплата морської доставки',partialSea,'$'),
   line(`SWIFT переказ (${SETTINGS.swiftPercent}%)`,swift.swift,'$')
 ];
 const secondRows=[
   line('Доставка до порту',land,'$'),
   line('Доставка по морю',sea,'$'),
   line('Вигрузка',SETTINGS.usaUnload,'$'),
   line('Доставка до України',SETTINGS.usaUkraine,'$')
 ];
 if(usaMode==='with') secondRows.push(line('Розмитнення',customs.total,'$'));
 secondRows.push(
   line('Брокерські послуги',SETTINGS.usaBroker,'$'),
   line('Вартість послуг з підбору та бронювання',SETTINGS.usaService,'$')
 );
 renderResult('usa-results','$',total,firstPayment,secondPayment,firstRows,secondRows);
}
...
function calculateEurope(){
 ...
 // Доставка по Європі та частина прибутку приховані у лоті/зборі, решта прибутку — у доставці до України.
 ...
 const swift=swiftGross(firstCommercial);
 const firstPayment=swift.total;
 const secondPayment=customsTotal;
 const total=firstPayment+secondPayment;

 const firstRows=[
   line('Вартість лоту + аукціонний збір',visibleLotAuction,'€'),
   line('Доставка до України',visibleUkraine,'€'),
   line('Брокерські послуги',SETTINGS.europeBroker,'€'),
   line('Вартість послуг з підбору та бронювання',SETTINGS.europeService,'€'),
   line('Сертифікація',SETTINGS.europeCertificate,'€'),
   line(`SWIFT переказ (${SETTINGS.swiftPercent}%)`,swift.swift,'€')
 ];
 const secondRows=europeMode==='with'?[line('Розмитнення',customs.total,'€')]:[];
 renderResult('eu-results','€',total,firstPayment,secondPayment,firstRows,secondRows);
}
